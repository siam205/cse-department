'use server';

import { revalidatePath } from 'next/cache';
import { redirect } from 'next/navigation';
import type { Prisma } from '@prisma/client';
import { prisma } from '@/lib/db';
import { getSession } from '@/lib/auth-server';
import { visitorCreateSchema, visitorUpdateSchema } from '@/lib/validation';

export type ActionResult = { ok: true } | { ok: false; error: string };

function getStr(fd: FormData, key: string): string {
  const v = fd.get(key);
  return typeof v === 'string' ? v.trim() : '';
}
function emptyToNull(v: FormDataEntryValue | null): string | null {
  if (typeof v !== 'string') return null;
  const t = v.trim();
  return t.length > 0 ? t : null;
}
function getParagraphs(fd: FormData, key: string): string[] {
  return fd.getAll(key)
    .filter((v): v is string => typeof v === 'string')
    .map((v) => v.trim())
    .filter((v) => v.length > 0);
}
async function requireAuth(): Promise<ActionResult | null> {
  const session = await getSession();
  if (!session?.user) return { ok: false, error: 'Not authenticated' };
  return null;
}

function revalidateVisitorSurfaces() {
  revalidatePath('/student-society/visitor');
  revalidatePath('/admin/visitors');
  revalidatePath('/admin');
  revalidatePath('/', 'layout');
}

function readVisitorRow(formData: FormData) {
  return {
    slug:          getStr(formData, 'slug'),
    name:          getStr(formData, 'name'),
    role:          emptyToNull(formData.get('role')),
    affiliation:   emptyToNull(formData.get('affiliation')),
    photoUrl:      getStr(formData, 'photoUrl'),
    photoPublicId: emptyToNull(formData.get('photoPublicId')),
    quote:         getParagraphs(formData, 'quote'),
  };
}

export async function createVisitorAction(
  _prev: ActionResult | { ok: null },
  formData: FormData,
): Promise<ActionResult> {
  const denied = await requireAuth();
  if (denied) return denied;

  const parsed = visitorCreateSchema.safeParse(readVisitorRow(formData));
  if (!parsed.success) {
    return { ok: false, error: parsed.error.issues.map((i) => `${i.path.join('.') || '(root)'}: ${i.message}`).join('; ') };
  }

  const last = await prisma.visitor.findFirst({ orderBy: { displayOrder: 'desc' }, select: { displayOrder: true } });
  const displayOrder = (last?.displayOrder ?? -1) + 1;

  try {
    await prisma.visitor.create({
      data: {
        ...parsed.data,
        quote: parsed.data.quote as Prisma.InputJsonValue,
        displayOrder,
      },
    });
  } catch (e: unknown) {
    if ((e as { code?: string })?.code === 'P2002') {
      return { ok: false, error: `slug "${parsed.data.slug}" is already in use` };
    }
    return { ok: false, error: e instanceof Error ? e.message : 'Database error' };
  }
  revalidateVisitorSurfaces();
  redirect('/admin/visitors');
}

export async function updateVisitorAction(
  id: string,
  _prev: ActionResult | { ok: null },
  formData: FormData,
): Promise<ActionResult> {
  const denied = await requireAuth();
  if (denied) return denied;

  const parsed = visitorUpdateSchema.safeParse(readVisitorRow(formData));
  if (!parsed.success) {
    return { ok: false, error: parsed.error.issues.map((i) => `${i.path.join('.') || '(root)'}: ${i.message}`).join('; ') };
  }

  try {
    await prisma.visitor.update({
      where: { id },
      data: {
        ...parsed.data,
        quote: parsed.data.quote as Prisma.InputJsonValue,
      },
    });
  } catch (e: unknown) {
    const code = (e as { code?: string })?.code;
    if (code === 'P2025') return { ok: false, error: 'Visitor not found' };
    if (code === 'P2002') return { ok: false, error: 'slug already in use' };
    return { ok: false, error: e instanceof Error ? e.message : 'Database error' };
  }
  revalidateVisitorSurfaces();
  revalidatePath(`/admin/visitors/${id}`);
  return { ok: true };
}

export async function deleteVisitorAction(id: string): Promise<ActionResult> {
  const denied = await requireAuth();
  if (denied) return denied;
  try {
    await prisma.visitor.delete({ where: { id } });
  } catch (e: unknown) {
    if ((e as { code?: string })?.code === 'P2025') return { ok: false, error: 'Visitor not found' };
    return { ok: false, error: e instanceof Error ? e.message : 'Database error' };
  }
  revalidateVisitorSurfaces();
  return { ok: true };
}

export async function reorderVisitorsAction(ids: string[]): Promise<ActionResult> {
  const denied = await requireAuth();
  if (denied) return denied;
  const existing = await prisma.visitor.findMany({ select: { id: true } });
  const existingIds = new Set(existing.map((r) => r.id));
  if (ids.length !== existingIds.size || !ids.every((id) => existingIds.has(id))) {
    return { ok: false, error: 'Reorder list must include exactly the existing visitors' };
  }
  try {
    await prisma.$transaction(
      ids.map((id, index) => prisma.visitor.update({ where: { id }, data: { displayOrder: index } })),
    );
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : 'Database error' };
  }
  revalidateVisitorSurfaces();
  return { ok: true };
}
