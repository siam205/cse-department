      ...
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-10 lg:gap-8 mb-12">
          ...
          {/* Mobile pairing — Quick Link + Campuses, matching the
              Useful Link / Get in Touch pair above. Both class strings
              are literals so Tailwind can see them; when no campuses
              are seeded the wrapper drops the 2-col grid so Quick Link
              keeps the full width instead of sitting in a half column. */}
                  ...
                  {campusLinks.map((link) => {
                    ...
                    return (
                      <li key={link.id}>
                        ...
                        >
                          {link.name}
                          {/* Inline (not a flex child) so it follows the
                              last word rather than being pushed to the
                              right edge. Hidden below sm: the mobile
                              column is 152px and "Panthapath Campus"
                              plus the arrow needs ~164px, which left the
                              arrow stranded on its own line. It is a
                              decorative external-link cue — the
                              aria-label carries the meaning. */}
                          <ArrowUpRight
                            size={14}
                            className="hidden sm:inline-block ml-1 align-[-2px] transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
                          />
                        </a>
                      </li>
                    );
