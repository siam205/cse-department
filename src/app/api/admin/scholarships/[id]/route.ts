...
import { scholarshipUpdateSchema } from '@/lib/validation';
...
export const GET = withErrorHandling(async (_request, context: RouteContext) => {
  ...
  if (!item) throw new ApiError(404, 'Scholarship not found');
...
export const PUT = withErrorHandling(async (request, context: RouteContext) => {
  ...
  try {
    const item = await prisma.scholarship.update({ where: { id }, data });
    return NextResponse.json({ item });
  } catch (e: unknown) {
    if ((e as { code?: string })?.code === 'P2025') throw new ApiError(404, 'Scholarship not found');
    throw e;
  }
});
