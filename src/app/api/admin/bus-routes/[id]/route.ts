...
import { busRouteUpdateSchema } from '@/lib/validation';
...
export const GET = withErrorHandling(async (_request, context: RouteContext) => {
  ...
  if (!route) throw new ApiError(404, 'Bus route not found');
...
export const PUT = withErrorHandling(async (request, context: RouteContext) => {
  ...
  const data = busRouteUpdateSchema.parse(body);
  try {
    const route = await prisma.busRoute.update({ where: { id }, data });
    return NextResponse.json({ route });
  } catch (e: unknown) {
    if ((e as { code?: string })?.code === 'P2025') throw new ApiError(404, 'Bus route not found');
    throw e;
  }
});
